				   ...
				   uint32_t start_addr, uint32_t end_addr)
{
	...
	LOG_INFO ("mips32_pracc_sync_cache");
    /* TODO remove array */
...
{
	LOG_INFO ("mips32_pracc_write_mem - addr: 0x%8.8x  size: %d, count: %d", addr, size, count);
    int retval = mips32_pracc_write_mem_generic(ejtag_info, addr, size, count, buf);
    if (retval != ERROR_OK)
		return retval;

	// Invalidate cache
	mips32_pracc_invalidate_cache (ejtag_info, addr, size, count);
...
//    LOG_DEBUG("%s using 0x%.8" PRIx32 " for write handler", __func__, source->address);
	...
	{
		LOG_INFO ("address != MIPS32_PRACC_FASTDATA_AREA");
		return ERROR_FAIL;
	}
